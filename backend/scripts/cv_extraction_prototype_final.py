...
Pipeline:
  ...
  CV file (PDF/DOCX)
     ...
     │
     ├─ [2] GLiNER zero-shot NER (all local, 1500-char chunks)
     │       skills, certs, companies, job titles, degrees
     │
     ├─ [3-7] LLM passes (with NER hints)
     │   ├─ Person identity (top 4k chars)
     │   ├─ Skills (sampled 8k chars)
     │   ├─ Employment + Projects (sampled 8k, employer/project split)
     │   ├─ Projects per-chunk (dedup by company + dates)
     │   └─ Certs & Education (per-chunk scan + signature dedup)
     │
     ├─ [8] Cert/skill dedup
     │
     └─ [9] Deterministic graph assembly (reified entities)
...
Usage:
  python backend/scripts/cv_extraction_prototype.py backend/onyx/data/John_Mendy.docx
...
def process_cv(path: Path) -> dict[str, Any]:
    ...
    # 6. LLM Pass 4 — projects per-chunk (CVs can have 50+ projects)
    print(f"\n[6] LLM: projects per-chunk ({len(parsed.chunks)} chunks)...")
    t0 = time.time()
    ...
    for chunk in chunks_to_scan_proj:
        if len(chunk.strip()) < 50:
            continue
        parsed_chunk = parse_json_from_llm(llm_extract(projects_prompt, chunk[:8000]))
        if not parsed_chunk:
            continue
    ...
    n_proj_total = len(existing_projects)
    print(f"  {time.time()-t0:.1f}s → {n_proj_total} projects total (chunk-scan added {n_proj_total - n_proj})")

    # 7. LLM Pass 5 — certs & education (per-chunk)
    print(f"\n[7] LLM: certs & education ({len(parsed.chunks)} chunks)...")
    t0 = time.time()
