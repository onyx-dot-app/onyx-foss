"""Tests for `get_json_with_retry`, the raw Microsoft Graph request path in the Teams
connector (`execute_request_direct`), used by `fetch_messages`/`fetch_replies`.

`get_json_with_retry` retries transient Graph errors (rate limits + 5xx gateway/server
...
def test_returns_json_without_retry_on_success() -> None:
    ...
    with patch("onyx.connectors.teams.utils.time.sleep") as mock_sleep:
        result = get_json_with_retry(graph_client=graph_client, request_url="teams")
...
def test_does_not_retry_non_retryable_status(status: int) -> None:
    ...
    with patch("onyx.connectors.teams.utils.time.sleep") as mock_sleep:
        with pytest.raises(HTTPError):
            get_json_with_retry(graph_client=graph_client, request_url="teams")
...
def test_raises_runtime_error_after_exhausting_retries() -> None:
    # Always returns a retryable status; get_json_with_retry caps at MAX_RETRIES (10) attempts
