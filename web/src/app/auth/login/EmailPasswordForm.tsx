        ...
        onSubmit={async (values: { email: string; password: string }) => {
          ...
          if (isSignup) {
            ...
            if (!response.ok) {
              ...
              setApiStatus("error");
              toast.error(t("auth.failedToSignup", { message: errorMsg }));
              setIsWorking(false);
              return;
            } else {
          ...
          if (loginResponse.ok) {
            ...
            if (errorDetail === "LOGIN_BAD_CREDENTIALS") {
              errorMsg = t("auth.invalidEmailOrPassword");
            } else if (errorDetail === "NO_WEB_LOGIN_AND_HAS_NO_PASSWORD") {
              errorMsg = t("auth.createAccountToSetPassword");
            ...
            setErrorMessage(errorMsg);
            setApiStatus("error");
            toast.error(t("auth.failedToLogin", { message: errorMsg }));
          }
        }}
      >
              ...
              <FormikField<string>
                ...
                render={(field, helper, meta, state) => (
                  <FormField name="email" state={state} className="w-full">
                    <FormField.Label>{t("auth.emailAddress")}</FormField.Label>
                    <FormField.Control>
                      <InputTypeIn
                        {...field}
                        onChange={(e) => {
                ...
                render={(field, helper, meta, state) => (
                  <FormField name="password" state={state} className="w-full">
                    ...
                    {isSignup && !showApiMessage && (
                      <FormField.Message
                        messages={{
                          idle: t("auth.passwordMinLength", {
                            count: passwordMinLength,
                          }),
                          error: meta.error,
                          success: t("auth.passwordMinLength", {
                            count: passwordMinLength,
                          }),
                        }}
                      />
